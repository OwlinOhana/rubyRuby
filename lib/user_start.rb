class UserStart
  ...
  def put_scale_in
    ...
    ErrorHandler.check_correct_scale(gets.chomp) == true ? put_scale_in : :@im_temp
  end

  def put_scale_om
    puts 'Введите единицу измерения,в которую необходимо перевести'
    @om_temp = gets.chomp
    ErrorHandler.check_correct_scale(gets.chomp) == true ? put_scale_in : :@om_temp
    ErrorHandler.check_repeating_scale(@im_temp, @om_temp)
  end
end
